...
export class BundleAnalyzer {
    ...
    private readonly recommendations: Array<AssetOptimizationRecommendation> = []
    ...
    public generateCodeSplittingRecommendations(): Array<CodeSplittingRecommendation> {
        ...
        const largeModules = this.bundleAnalysis.modules
    ...
    private analyzeVendorBundle(): VendorAnalysis {
        ...
        const vendorAssets = this.bundleAnalysis.assets.filter(asset => 
    ...
    private analyzeAppBundle(): AppAnalysis {
        ...
        const appAssets = this.bundleAnalysis.assets.filter(asset => 
            !asset.name.includes('vendor') && !asset.name.includes('node_modules'),
        )
    ...
    private analyzeChunks(): ChunkBreakdown {
        return {
            ...
            largestChunks: this.bundleAnalysis.chunks
                .sort((a, b) => b.size - a.size)
                .slice(0, 10),
    ...
    private getGradeAndStatus(score: number): { grade: 'A' | 'B' | 'C' | 'D' | 'E' | 'F', status: 'excellent' | 'good' | 'fair' | 'poor' | 'critical' } {
    ...
    private calculateCodeSplittingPotential(modules: Array<ModuleAnalysis>): number {
        const routeModules = modules.filter(m => 
            m.name.includes('page') || m.name.includes('route') || m.name.includes('view'),
        )
        return (routeModules.length / modules.length) * 100
    }

    private calculateChunkEfficiency(): number {
        const avgChunkSize = this.bundleAnalysis.chunks.reduce(
            (sum, chunk) => sum + chunk.size, 0,
        ) / this.bundleAnalysis.chunks.length
    ...
    private calculateImmediateOptimizationPotential(): number {
        ...
        const compressibleAssets = this.bundleAnalysis.assets.filter(a => 
            a.type === 'image' || a.type === 'css',
        )
    ...
    private calculateMediumOptimizationPotential(): number {
        ...
        const splittableModules = this.bundleAnalysis.modules.filter(m => 
            m.size > 50 * 1024 && !m.isVendor,
        )
